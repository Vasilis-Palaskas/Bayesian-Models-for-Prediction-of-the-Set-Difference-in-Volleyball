...
train_dataList<-list(n_teams=length(teams_names),n_games=length(datafr_teams_scores_set_win$home_score)+length(quarter_train_away_sets)+
...
test_dataList<-list(n_teams=length(teams_names),n_games=length(c(7,12,7,12,7)),
					home_team=c(7,12,7,12,7),
                        away_team=c(12,7,12,7,12))
...
#-------------------------------------------------------------------------------------------------------------------------------
########-------------- 1) Final ZDTS (Paper v1) (Finals)--------------#################


Finals_Final_ZDTS_paper.v1<-stan(file.choose(),data=train_dataList,chains=3,thin=5,
iter=40000,warmup=10000,cores=3,seed="12345",init_r=1,verbose=T)####Run Stan Models/final_ZDTS_paper.v1.stan


#save(Finals_Final_ZDTS_paper.v1,file="Finals-final Final ZDTS (Paper v1)")##call with data, attach, load

#load(file.choose())# #Output/Stan Models/Finals-final Final ZDTS (Paper v1)




model<-Finals_Final_ZDTS_paper.v1

train_dataset<-train_dataList
test_dataset<-test_dataList
observ_qualified_teams<-c(12)

#-------------------------------------------------------------
####-------------FUNCTION--------------#####

Finals_prediction_ZDTS<-function(train_dataset,test_dataset,model,
...
#columns are the whole number of the league teams
  ...
  matrix_pred_game_points<-matrix(nrow=matrix_rows,ncol=train_dataset$n_teams)
...
Finals_results_ZDTS<-Finals_prediction_ZDTS(train_dataset,test_dataset,
