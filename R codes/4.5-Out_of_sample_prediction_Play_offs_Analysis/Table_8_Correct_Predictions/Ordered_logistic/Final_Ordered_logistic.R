...
train_dataList_new<-list(n_teams=12,n_games=length(datafr_teams_scores_set_win$home_Team)+length(quarter_train_home_sets)+length(semi_train_home_sets),
                         dif_sets=c(datafr_teams_scores_set_win$home_score-datafr_teams_scores_set_win$away_score,
                                    quarter_train_home_sets-quarter_train_away_sets,semi_train_home_sets-semi_train_away_sets),
                         home_team=c(as.numeric(factor(datafr_teams_scores_set_win$home_Team)),quarter_train_home_team,
                                     semi_train_home_team   ),  
                         away_team=c(as.numeric(factor(datafr_teams_scores_set_win$away_Team)),
                                     quarter_train_away_team,semi_train_away_team))
...
test_dataList_new_final<-list(n_teams=12,n_games=length(c(7,12,7,12,7)),
...
rstan_ordered_final<-stan(file.choose(),data=model_dataList_new_final,chains=3,thin=2,
                            iter=16000,warmup=4000,cores=3)####Run out_of_sample_Ordered_paper.v1.stan
#save(rstan_ordered_final,file="Finals-final ordered")##call with data, attach, load


#load(file.choose())# #Output/Stan Models/Finals-final ordered



model<-rstan_ordered_final
...
finals_final_prediction_ordered<-function(dataset,model,
...
final_finals_results_ordered<-finals_final_prediction_ordered(model_dataList_new_final,
                                                         rstan_ordered_final,observ_qualified_teams)

final_finals_results_ordered$total_percentage_qualification#21.41
